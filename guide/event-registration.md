...
- **Registration Opens / Closes** determine when the registartion is open or closed. These obey the event timezone configured.
- When outside registration timeframe, the registartion is automatically disabled and prevents new submissions while keeping existing registrations editable (until the edit cut-off).
...
- **Capacity**: global attendee cap for the event. When reached, the form blocks new registartions.
- **Ticket Types**: represent your pricing tiers (e.g., Member, Guest, VIP). For each ticket type:
  ...
  - Tie add-ons (shirts, meals, merch) to a ticket type when they’re required and included for free or optional at additional cost.
...
- Use **Available Add-ons** to collect add-ons and variant choices (shirt sizes, transport slots, merch bundles).
